/**
 * RetroWebLauncher - Header Component (Lit)
 * Displays arcade name, search, and navigation
 */

import { state } from '../state.js';
import { router } from '../router.js';

const { LitElement, html, css } = window.Lit;

class RwlHeader extends LitElement {
  static properties = {
    _isFullscreen: { state: true },
    _connected: { state: true },
    _arcadeName: { state: true },
    _isPWA: { state: true }
  };

  static styles = css`
    :host {
      display: block;
      height: var(--header-height, 64px);
      /* Add safe area padding for iOS PWA - status bar overlaps in standalone mode */
      padding-top: env(safe-area-inset-top, 0px);
      background: var(--header-background, linear-gradient(180deg, rgba(0,0,0,0.9) 0%, rgba(0,0,0,0.7) 100%));
      border-bottom: 2px solid var(--header-border-color, var(--color-primary, #ff0066));
      position: relative;
      z-index: var(--z-header, 200);
      backdrop-filter: blur(10px);
      -webkit-backdrop-filter: blur(10px);
      /* Ensure the background extends behind the status bar */
      box-sizing: content-box;
    }

    .header-container {
      display: flex;
      align-items: center;
      justify-content: space-between;
      height: 100%;
      padding: 0 var(--spacing-lg, 1.5rem);
      max-width: 100%;
    }

    .header-left {
      display: flex;
      align-items: center;
      gap: var(--spacing-md, 1rem);
    }

    .home-btn {
      background: none;
      border: none;
      color: var(--header-text-color, var(--color-text, #fff));
      cursor: pointer;
      padding: var(--spacing-sm, 0.5rem);
      border-radius: var(--radius-md, 8px);
      transition: background var(--transition-fast, 150ms);
    }

    .home-btn:hover {
      background: var(--header-btn-background-hover, rgba(255,255,255,0.1));
    }

    .home-btn svg {
      width: 24px;
      height: 24px;
    }

    .arcade-name {
      font-family: var(--font-display, 'VT323', monospace);
      font-size: var(--font-size-lg, 1.25rem);
      color: var(--color-primary, #ff0066);
      text-shadow: var(--arcade-name-glow,
        0 0 10px var(--arcade-name-glow-color, rgba(255, 0, 102, 0.5)),
        0 0 20px var(--arcade-name-glow-color, rgba(255, 0, 102, 0.3)));
      margin: 0;
      letter-spacing: 0.05em;
    }

    .header-center {
      flex: 1;
      display: flex;
      justify-content: center;
    }

    .header-right {
      display: flex;
      align-items: center;
      gap: var(--spacing-sm, 0.5rem);
    }

    .header-btn {
      display: flex;
      align-items: center;
      gap: var(--spacing-xs, 0.25rem);
      background: var(--header-btn-background, rgba(255,255,255,0.1));
      border: 1px solid var(--header-btn-border, rgba(255,255,255,0.2));
      border-radius: var(--radius-md, 8px);
      padding: var(--spacing-sm, 0.5rem) var(--spacing-md, 1rem);
      color: var(--header-text-color, var(--color-text, #fff));
      font-family: var(--font-body);
      font-size: var(--font-size-sm, 0.75rem);
      cursor: pointer;
      transition: all var(--transition-fast, 150ms);
    }

    .header-btn:hover {
      background: var(--header-btn-background-hover, rgba(255,255,255,0.2));
      border-color: var(--color-primary, #ff0066);
    }

    .header-btn:focus-visible {
      outline: var(--focus-ring-width, 4px) solid var(--focus-ring-color, #ff0066);
      outline-offset: 2px;
    }

    .header-btn svg {
      width: 16px;
      height: 16px;
    }

    .connection-status {
      display: flex;
      align-items: center;
      gap: var(--spacing-xs, 0.25rem);
      font-size: var(--font-size-xs, 0.625rem);
      color: var(--color-success, #00ff66);
    }

    .connection-status.disconnected {
      color: var(--color-error, #ff3333);
    }

    .status-dot {
      width: 8px;
      height: 8px;
      border-radius: 50%;
      background: var(--color-success, #00ff66);
      box-shadow: 0 0 8px var(--color-success, #00ff66);
      animation: pulse 2s infinite;
    }

    .connection-status.disconnected .status-dot {
      background: var(--color-error, #ff3333);
      box-shadow: 0 0 8px var(--color-error, #ff3333);
      animation: none;
    }

    @keyframes pulse {
      0%, 100% { opacity: 1; }
      50% { opacity: 0.5; }
    }

    @media (max-width: 768px) {
      .arcade-name {
        font-size: var(--font-size-base, 1rem);
      }

      .header-btn span {
        display: none;
      }

      .header-btn {
        padding: var(--spacing-sm, 0.5rem);
      }
    }
  `;

  constructor() {
    super();
    this._isFullscreen = false;
    this._connected = false;
    this._arcadeName = 'RetroWebLauncher';
    this._isPWA = false;
    this._unsubscribers = [];
    this._boundFullscreenHandler = () => this._updateFullscreen();
    // Detect Safari on iOS/iPadOS - limited fullscreen support
    this._isSafariIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) ||
                        (navigator.userAgent.includes('Mac') && 'ontouchend' in document);
  }

  connectedCallback() {
    super.connectedCallback();

    this._unsubscribers.push(
      state.subscribe('config', (config) => {
        if (config?.arcadeName) {
          this._arcadeName = config.arcadeName;
        }
      })
    );

    this._unsubscribers.push(
      state.subscribe('connected', (connected) => {
        this._connected = connected;
      })
    );

    document.addEventListener('fullscreenchange', this._boundFullscreenHandler);
    document.addEventListener('webkitfullscreenchange', this._boundFullscreenHandler);

    this._connected = state.get('connected');
    const config = state.get('config');
    if (config?.arcadeName) {
      this._arcadeName = config.arcadeName;
    }

    // Check if running as PWA (hide fullscreen button in standalone mode)
    const pwa = state.get('pwa');
    this._isPWA = pwa?.isStandalone || false;
  }

  disconnectedCallback() {
    super.disconnectedCallback();
    this._unsubscribers.forEach(unsub => unsub());
    this._unsubscribers = [];
    document.removeEventListener('fullscreenchange', this._boundFullscreenHandler);
    document.removeEventListener('webkitfullscreenchange', this._boundFullscreenHandler);
  }

  _updateFullscreen() {
    this._isFullscreen = !!(document.fullscreenElement || document.webkitFullscreenElement);
  }

  _toggleFullscreen() {
    // On iOS/iPadOS Safari, fullscreen API doesn't work for document
    // Recommend using "Add to Home Screen" for PWA standalone mode instead
    if (this._isSafariIOS && !document.fullscreenElement && !document.webkitFullscreenElement) {
      // Check if running as PWA (standalone mode)
      const isStandalone = window.navigator.standalone ||
                           window.matchMedia('(display-mode: standalone)').matches;
      if (!isStandalone) {
        console.info('Fullscreen not fully supported on Safari. For best experience, add to Home Screen.');
      }
    }

    if (!document.fullscreenElement && !document.webkitFullscreenElement) {
      const elem = document.documentElement;
      if (elem.requestFullscreen) {
        elem.requestFullscreen().catch(err => {
          console.warn('Fullscreen request failed:', err);
        });
      } else if (elem.webkitRequestFullscreen) {
        elem.webkitRequestFullscreen();
      }
    } else {
      if (document.exitFullscreen) {
        document.exitFullscreen();
      } else if (document.webkitExitFullscreen) {
        document.webkitExitFullscreen();
      }
    }
  }

  render() {
    return html`
      <div class="header-container">
        <div class="header-left">
          <button class="home-btn" title="Home" aria-label="Go to home screen" @click=${() => router.navigate('/')}>
            <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/>
            </svg>
          </button>
          <h1 class="arcade-name">${this._arcadeName}</h1>
        </div>

        <div class="header-center"></div>

        <div class="header-right">
          <div class="connection-status ${this._connected ? '' : 'disconnected'}" role="status" aria-live="polite">
            <span class="status-dot" aria-hidden="true"></span>
            <span>${this._connected ? 'Connected' : 'Disconnected'}</span>
          </div>

          <button class="header-btn" title="Search (Y)" aria-label="Open search" @click=${() => router.navigate('/search')}>
            <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M15.5 14h-.79l-.28-.27a6.5 6.5 0 0 0 1.48-5.34c-.47-2.78-2.79-5-5.59-5.34a6.505 6.505 0 0 0-7.27 7.27c.34 2.8 2.56 5.12 5.34 5.59a6.5 6.5 0 0 0 5.34-1.48l.27.28v.79l4.25 4.25c.41.41 1.08.41 1.49 0 .41-.41.41-1.08 0-1.49L15.5 14zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/>
            </svg>
            <span>Search</span>
          </button>

          ${!this._isPWA ? html`
            <button class="header-btn" title="${this._isFullscreen ? 'Exit Fullscreen (F11)' : 'Fullscreen (F11)'}" aria-label="Toggle fullscreen mode" @click=${this._toggleFullscreen}>
              ${this._isFullscreen ? html`
                <svg viewBox="0 0 24 24" fill="currentColor">
                  <path d="M5 16h3v3h2v-5H5v2zm3-8H5v2h5V5H8v3zm6 11h2v-3h3v-2h-5v5zm2-11V5h-2v5h5V8h-3z"/>
                </svg>
                <span>Exit Fullscreen</span>
              ` : html`
                <svg viewBox="0 0 24 24" fill="currentColor">
                  <path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/>
                </svg>
                <span>Fullscreen</span>
              `}
            </button>
          ` : ''}

          <button class="header-btn" title="Settings" aria-label="Open settings" @click=${() => router.navigate('/settings')}>
            <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M19.14 12.94c.04-.31.06-.63.06-.94 0-.31-.02-.63-.06-.94l2.03-1.58a.49.49 0 0 0 .12-.61l-1.92-3.32a.49.49 0 0 0-.59-.22l-2.39.96c-.5-.38-1.03-.7-1.62-.94l-.36-2.54a.484.484 0 0 0-.48-.41h-3.84c-.24 0-.43.17-.47.41l-.36 2.54c-.59.24-1.13.57-1.62.94l-2.39-.96a.49.49 0 0 0-.59.22L2.74 8.87c-.12.21-.08.47.12.61l2.03 1.58c-.04.31-.06.63-.06.94s.02.63.06.94l-2.03 1.58a.49.49 0 0 0-.12.61l1.92 3.32c.12.22.37.29.59.22l2.39-.96c.5.38 1.03.7 1.62.94l.36 2.54c.05.24.24.41.48.41h3.84c.24 0 .44-.17.47-.41l.36-2.54c.59-.24 1.13-.56 1.62-.94l2.39.96c.22.08.47 0 .59-.22l1.92-3.32c.12-.22.07-.47-.12-.61l-2.01-1.58zM12 15.6c-1.98 0-3.6-1.62-3.6-3.6s1.62-3.6 3.6-3.6 3.6 1.62 3.6 3.6-1.62 3.6-3.6 3.6z"/>
            </svg>
            <span>Settings</span>
          </button>
        </div>
      </div>
    `;
  }
}

customElements.define('rwl-header', RwlHeader);
