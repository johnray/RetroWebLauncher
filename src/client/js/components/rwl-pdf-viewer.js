/**
 * RetroWebLauncher - PDF Viewer Component (Lit)
 * Displays game manuals in PDF format
 */

const { LitElement, html, css } = window.Lit;

class RwlPdfViewer extends LitElement {
  static properties = {
    src: { type: String },
    _loading: { state: true },
    _error: { state: true }
  ...
  static styles = css`
    ...
    .pdf-frame {
      ...
      height: 100%;
      border: none;
      background: var(--pdf-frame-bg, #fff);
    }
  ...
  updated(changedProperties) {
    if (changedProperties.has('src') && this.src) {
      this._loading = true;
      this._error = false;
      this._setupFrameHandlers();
    }
  }

  _setupFrameHandlers() {
    this.updateComplete.then(() => {
      const frame = this.renderRoot.querySelector('.pdf-frame');
      if (frame) {
        frame.onload = () => {
          this._loading = false;
        };
      ...
      <div class="pdf-container">
        <div class="pdf-toolbar">
          ...
          <div class="toolbar-actions">
            <a class="toolbar-btn" href="${this.src}" target="_blank" rel="noopener" title="Open in new tab">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19 19H5V5h7V3H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7h-2v7zM14 3v2h3.59l-9.83 9.83 1.41 1.41L19 6.41V10h2V3h-7z"/>
              </svg>
              Open
            </a>
            <a class="toolbar-btn" href="${this.src}" download title="Download PDF">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div class="pdf-content">
          ...
          ${this._error ? html`
            <div class="error-state">
              ...
              <a class="download-link" href="${this.src}" target="_blank" rel="noopener">
          ...
          ${this.src && !this._error ? html`
            <iframe
              class="pdf-frame"
              src="${this.src}#view=FitH"
              title="Game Manual PDF"
              loading="lazy"
              style="${this._loading ? 'opacity: 0' : ''}"
            ></iframe>
          ` : ''}
        </div>
      </div>
